...
BASE_DATA_ROOT="data/output/tests/mmgm_single_fixed_proxy_campaign"
BASE_IMG_ROOT="img/tests/mmgm_single_fixed_proxy_campaign"
BASE_GP_ROOT="scripts/gnuplot/tests/mmgm_single_fixed_proxy_campaign"
...
for AFM_PRIORS in "${AFM_PRIOR_FILES[@]}"; do
  ...
  campaign_name="$(
    python3 - "$AFM_PRIORS" "$POPULATION_SIZE" <<'PY'
import json
import sys
...
try:
    radius_proxy_name = raw["source"]["radius_proxy_name"]
    strategy_name = raw["strategy"]["name"]
except KeyError as exc:
    raise SystemExit(f"Missing AFM prior metadata in {prior_path}: {exc}") from exc
if not isinstance(radius_proxy_name, str) or not radius_proxy_name:
    raise SystemExit(f"Invalid source.radius_proxy_name in {prior_path}")
if not isinstance(strategy_name, str) or not strategy_name:
    raise SystemExit(f"Invalid strategy.name in {prior_path}")
print(f"{radius_proxy_name}__{strategy_name}__fixed_pop_{population_size}")
PY
  )"
  DATA_ROOT="${BASE_DATA_ROOT}/${campaign_name}"
  IMG_ROOT="${BASE_IMG_ROOT}/${campaign_name}"
  GP_ROOT="${BASE_GP_ROOT}/${campaign_name}"
  ...
  for generation in "${GENERATIONS[@]}"; do
    for seed in "${SEEDS[@]}"; do
      ...
      echo "==> Fixed-AFM proxy campaign: prior=${prior_name}, output=${campaign_name}, generation=${generation}, seed=${seed}"
...
for data_root in sorted(path for path in base_data_root.iterdir() if path.is_dir()):
    ...
    proxy_aware_name = f"{radius_proxy_name}__{strategy_name}__{mode}_pop_{population_size}"
...
lines = ["=== FIXED-AFM PRIOR PROXY COMPARISON ===", ""]
for row in rows:
    lines.extend(
        [
