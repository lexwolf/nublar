...
BASE_DATA_ROOT="data/output/tests/mmgm_single_thesis_prior_campaign"
BASE_IMG_ROOT="img/tests/mmgm_single_thesis_prior_campaign"
BASE_GP_ROOT="scripts/gnuplot/tests/mmgm_single_thesis_prior_campaign"
STATUS_FILE="${BASE_DATA_ROOT}/status.txt"

STRATEGIES=(fixed hard soft softer)
...
echo "=== thesis-prior MMGM campaign started: $(timestamp) ===" >> "$STATUS_FILE"
...
for strategy in "${STRATEGIES[@]}"; do
  ...
  THESIS_PRIORS="${THESIS_PRIOR_DIR}/mmgm_single_thesis_${strategy}.json"
  ...
  for generation in "${GENERATIONS[@]}"; do
    for seed in "${SEEDS[@]}"; do
      ...
      status_prefix="strategy:${strategy_index}/${strategy_total} strategy_name:${strategy} seed:${seed} generation:${generation} run:${run_index}/${run_total}"
      ...
      echo "==> Thesis-prior campaign: strategy=${strategy}, mode=${thesis_mode}, generation=${generation}, seed=${seed}"
      set +e
      python3 tools/optimal/optimize_global_model_parameters.py \
        "$MODEL_FLAG" \
        --thesis-priors-json "$THESIS_PRIORS" \
        --thesis-priors-mode "$thesis_mode" \
        --seed "$seed" \
  ...
  if [ "$n_results" -eq 0 ]; then
    echo "strategy:${strategy_index}/${strategy_total} strategy_name:${strategy} status:analyzer_skipped reason:no_successful_runs timestamp:$(timestamp)" >> "$STATUS_FILE"
    continue
  fi

  analyzer_prefix="strategy:${strategy_index}/${strategy_total} strategy_name:${strategy}"
...
comparison_csv = base_data_root / "thesis_prior_comparison.csv"
comparison_report = base_data_root / "thesis_prior_comparison_report.txt"

rows: list[dict[str, str]] = []
report_sections: list[str] = []
for strategy in strategies:
    data_root = base_data_root / f"{strategy}_pop_{population_size}"
...
lines = ["MMGM THESIS-PRIOR CAMPAIGN", "", *report_sections]
